...
### Fixed
- **Empty model output no longer produces blank/0-byte subtitles** — A missing *or whitespace-only* translation now counts as a failed position (retry + original-text fallback + `Partial` status) instead of a blank cue marked as success. The regex JSON fallback also skips empty captures instead of fabricating them.
...
### Fixed
- **`Directory is not visible inside the container` spam + stale entries never healed** — A media whose directory is gone (moved, renamed, deleted, or mount missing) logged the same warning for *every episode, every pass* and never recovered. Now: the warning is logged once per path per run, the skip is a distinct `missing directory` figure in the pass summaries, and the automation self-heals — the movie/show is re-synced from Radarr/Sonarr (moved/renamed paths are refreshed, entries removed at the source are deleted from Lingarr so a later resync recreates them). Resyncs are capped per run (25) so a whole missing mount does not stampede the API; if Sonarr/Radarr is unreachable the entries are kept and the log says so.
